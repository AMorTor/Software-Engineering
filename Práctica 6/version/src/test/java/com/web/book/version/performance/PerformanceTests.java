...
public class PerformanceTests {
    ...
    public static void setupJMeter() {
        try {
            ...
            // Add these lines
            //SaveService.setIgnoreResources(false);
            System.setProperty("jmeter.save.saveservice.output_format", "xml");
            System.setProperty("sun.reflect.debugModuleAccess", "true");
    ...
    public void testHomePagePerformance() throws Exception {
        ...
        // In testHomePagePerformance() method, replace the result collection part:

        // Configure result collector with detailed metrics
        ResultCollector resultCollector = new ResultCollector(listener);
        resultCollector.setFilename("target/jmeter/results/homepage-test-" + 
            System.currentTimeMillis() + ".jtl");

        // Enable detailed result saving
        ...
        new File("target/jmeter/results").mkdirs();
        // Replace the SaveService.saveTree line with:
        try {
            String outputFile = "target/jmeter/test-plan.jmx";
            try (FileOutputStream os = new FileOutputStream(outputFile)) {
                SaveService.saveTree(testPlanTree, os);
            }
        } catch (Exception e) {
            // Log the error but continue with the test
            System.err.println("Warning: Could not save test plan: " + e.getMessage());
        }
